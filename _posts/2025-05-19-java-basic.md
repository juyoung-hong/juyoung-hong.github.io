...
- 생성자는 메소드와 비슷하게 생겼으나, 리턴 타입이 없으며 클래스 이름으로 되어 있음
- 기본 생성자는 다른 생성자가 없는 경우 기본으로 컴파일할 때 만들어지지만, 다른 생성자가 있는 경우는 자동으로 만들어지지 않음
- 객체의 변수와 매개 변수의 이름이 동일할 때, 인스턴스의 변수를 구분하기 위해서 this 예약어를 사용함

**생성자의 위치:** <br>
생성자를 작성하는 경우는 클래스의 다른 메소드들보다 위에, 가장 윗부분에 선언하는 것이 좋음 (인스턴스 변수 > 생성자 > 메소드 순) <br>
{: .notice--info}

### 메소드

- 오버로딩: 메소드의 이름은 같게 하고, 매개 변수들을 다르게 하는 방법 (매개변수의 타입이 다르면 다른 메소드로 인식함)
- 같은 역할을 하는 메소드는 같은 메소드 이름을 가져야 한다는 모토로 사용함
- return 예약어를 사용하면, 메소드는 값을 넘겨주고 그 위치에서 종료됨: 한개의 타입만 리턴할 수 있으므로 DTO를 사용함
- void 메소드에 대해서 더 이상 실행하고 싶지 않을때는 "return;" 구문을 통해 메소드 수행을 종료할 수 있음
- static: 객체를 생성하지 않아도 메소드를 호출할 수 있는 메소드 **(static은 원치 않는 결과를 초래할 수 있으므로 아무때나 붙이면 안됨)**
- 매개 변수의 개수가 정해지지 않은 경우 아래와 같이 사용할 수 있음

```java
public void calculateNumbers(int...numbers){
  // "타입...변수명"으로 사용함: 변수는 배열을 전달받은 것과 다름없이 사용할 수 있음
  // 하나의 메소드에서는 한번만 사용 가능하며, 여러 매개변수가 있다면 가장 마지막에 선언해야함
}
```

### static 블록

- 어떤 클래스의 객체가 생성되면서 딱 한번만 불려야 하는 코드가 있는 경우, static 블록을 사용함

```java
static {
  // 딱 한번만 수행되는 코드: 객체 생성 전 한번만 호출되고, 그 이후에는 호출되지 않음. 클래스 내에 선언되어 있어야 하며, 메소드 내에서는 선언할 수 없음
  // static block은 여러개 선언할 수 있으며 선언된 순서대로 차례로 호출됨. static block에서는 static 한 것들만 호출할 수 있음
}
```

### Pass by value, Pass by reference

- Pass by value: 메소드의 매개변수로 넘길때 원래의 값은 그대로 두고, 전달되는 값이 진짜인 것처럼 함
- 기본자료형은 전부 Pass by Value로 전달하며, 참조 자료형은 Pass by Reference로 데이터를 전달 함: Pass by Reference로 전달되면 호출한 메소드의 데이터에도 영향이 있음
- 참조 자료형도 호출된 메소드에서 다른 객체를 생성하여 대체하면, 기존 데이터는 바뀌지 않음
